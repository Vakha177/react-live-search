...
import style from '../Main/Goods.module.css'
import { useState } from 'react'
  function Main() {
    const product = [
      ...
      const [productt , setProduct] = useState(product)
      const filteredProduct = productt.filter(product => {
        return product.title.toLowerCase().includes(value.toLowerCase())
      }) 

    return(
        <div className={style.contayner}>
            <input placeholder='Live Search'
             type="text" 
             className={style.input}
             value={value}
            onChange={(e) => setValue(e.target.value)}
            />
             {filteredProduct.map(item =><Goods image={item.image} cost={item.cost} title={item.title} button={item.button} cost1={item.cost1}/> )}
        </div>
    )
  }
